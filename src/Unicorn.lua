...
function Unicorn:draw(backgroundX, backgroundY)
	...
	love.graphics.draw(self.img[self.currentImgIndex], self.realX, self.realY, 0, self.orientation, 1, xOffset, yOffset)
end

function Unicorn:update(dt, astronaut, mapImg)
...
function Unicorn:move(dt, mapImg)
	...
	if self.isWalking == false then
		...
		if self.idleTime == false then
			if self.direction == 0 and self.realX < mapImg:getWidth() - self.img[self.currentImgIndex]:getWidth()/2  then
				self.x = self.x + dt*self.speed
			elseif self.direction == 1 and self.realY < mapImg:getHeight() - self.img[self.currentImgIndex]:getHeight()/2 then
				self.y = self.y + dt*self.speed
			elseif self.direction == 2 and self.realX > 0 + self.img[self.currentImgIndex]:getWidth()/2 then
				self.x = self.x - dt*self.speed
			elseif self.direction == 3 and self.realY > 0 + self.img[self.currentImgIndex]:getHeight()/2 then
				self.y = self.y - dt*self.speed
			end
		end
...
function Unicorn:checkPredator(playerX, playerY)
	if self.direction == 0 and math.abs(self.realY-playerY) < 60 and playerX-self.realX > 0 and playerX-self.realX < self.sightDistance then
		--Encontrou o player no leste
		self.direction = 2
		self.orientation = -1
		self.distanceFinal = 300
		self.speed = 200+self.baseSpeed
		self.distanceCounter = 0
		self.isWalking = true
		self.idleTime = false
	elseif self.direction == 1 and math.abs(self.realX-playerX) < 60 and playerY-self.realY > 0 and playerY-self.realY < self.sightDistance then
		...
		self.isWalking = true
		self.idleTime = false
	elseif self.direction == 2 and math.abs(self.realY-playerY) < 60 and playerX-self.realX < 0 and self.realX-playerX < self.sightDistance then
		...
		self.isWalking = true
		self.idleTime = false
	elseif self.direction == 3 and math.abs(self.realX-playerX) < 60 and playerY-self.realY < 0 and self.realY-playerY < self.sightDistance then
